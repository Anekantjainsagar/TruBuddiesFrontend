...
const Library = () => {
  ...
  useEffect(() => {
    if (context?.eventCategory?.length > 0) {
      const id = context?.eventCategory[0]?._id;
  ...
  const saveBook = () => {
    if (
      ...
      axios
        .post(`${BASE_URL}/services/social-support/add-support`, {
          ...data,
          thumbnail,
          time: sTime + "-" + eTime,
        })
        .then((res) => {
          if (res.status === 200) {
            ...
            context?.setEvents([...context?.events, res.data]);

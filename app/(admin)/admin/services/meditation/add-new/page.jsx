...
const MeditationAdmin = () => {
  ...
  const saveYoga = () => {
    if (data?.thumbnail && data?.poses && data?.title) {
      axios
        .post(`${BASE_URL}/services/meditation/`, { ...data })
        .then((response) => {
          if (response.status === 201) {
            toast.success("Meditation Position Added Successfully");
            history.push("/admin/services/meditation");
            context?.setMeditations([...context?.meditations, response.data]);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  return (
    <div className={`h-[100vh] ${noto_sans.className} px-5 overflow-y-auto`}>
      <div className="flex items-center justify-between py-3">
        <h1 className="text-2xl font-semibold">Add New Meditation</h1>
      </div>

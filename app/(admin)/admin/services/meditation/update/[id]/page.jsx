...
const UpdateMeditation = ({ params }) => {
  ...
  const saveYoga = () => {
    if (data?.thumbnail && data?.poses && data?.title) {
      axios
        .put(`${BASE_URL}/services/meditation/${id}`, { ...data })
        .then((response) => {
          if (response.status === 200) {
            context?.setMeditations(
              context?.meditations.map((med) =>
                med._id === id ? response.data : med
              )
            );
            toast.success("Meditation Position Added Successfully");
            history.push("/admin/services/meditation");
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  useEffect(() => {
    if (context?.meditations) {
      let meditation = context?.meditations?.find((e) => e?._id == id);
      setData({
        thumbnail: meditation?.thumbnail,
        title: meditation?.title,
        poses: meditation?.poses,
      });
    }
  }, [context?.meditations, id]);

  return (
    <div className={`h-[100vh] ${noto_sans.className} px-5 overflow-y-auto`}>
      <div className="flex items-center justify-between py-3">
        <h1 className="text-2xl font-semibold">Update Meditation</h1>
        <AiOutlineDelete
          onClick={async () => {
            let bool = window.confirm("Delete the item?");
            if (bool) {
              const res = await axios.delete(`${BASE_URL}/services/meditation/${id}`);
              if (res.status == 200) {
                context?.setMeditations(
                  context?.meditations.filter((med) => med._id !== id)
                );
                history.push("/admin/services/meditation");
      ...
      >
        Save Meditation Position
      </button>
    </div>
  );
};

const Block = ({ data, setOriginalData, originalData }) => {
  return (
  ...
  return (
    <div className="w-full flex items-start justify-between p-2 rounded-md border border-gray-100/90">
      ...
      <div className="w-[75%]">
        <input
          type="text"
          placeholder="Meditation Position Name"
          className="w-full outline-none border px-3 py-1 rounded-md"
        ...
        <textarea
          ...
          placeholder="Meditation Position Description"
          rows={5}

...
const UpdateYoga = ({ params }) => {
  ...
  const saveYoga = () => {
    if (data?.thumbnail && data?.poses && data?.title) {
      axios
        .put(`${BASE_URL}/services/yoga/${id}`, { ...data })
        .then((response) => {
          if (response.status === 200) {
            context?.setYogas(
              context?.yogas.map((yoga) =>
                yoga._id === id ? response.data : yoga
              )
            );
  ...
  useEffect(() => {
    if (context?.yogas) {
      let yoga = context?.yogas?.find((e) => e?._id == id);
      setData({
  ...
  return (
    <div className={`h-[100vh] ${noto_sans.className} px-5 overflow-y-auto`}>
      <div className="flex items-center justify-between py-3">
        <h1 className="text-2xl font-semibold">Update Yoga</h1>
        <AiOutlineDelete
          onClick={async () => {
            let bool = window.confirm("Delete the item?");
            if (bool) {
              const res = await axios.delete(`${BASE_URL}/services/yoga/${id}`);
              if (res.status == 200) {
                context?.setYogas(
                  context?.yogas.filter((yoga) => yoga._id !== id)
                );
                history.push("/admin/services/yoga");
